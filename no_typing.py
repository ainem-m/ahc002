...
class TileState:
    ...
    """
    def __init__(self, end_turn, pos, turn=0, seen=[], steps = [], output="", game_score=0, first_action=INF):
    ...
    def __str__(self):
        ...
        for i in range(1, self.turn_):
            ...
            path[h][w] = dic[dir]
        # 出力パート
        isConnectHorizontal = lambda h, w: w+1<TILE_SIZE and tiles[h][w] == tiles[h][w+1]
        isConnectVertical = lambda h, w: h+1<TILE_SIZE and tiles[h][w] == tiles[h+1][w]
...
def chokudaiSearchActionWithTimeThreshold(state, beam_width, beam_depth, time_threshold):
    """
