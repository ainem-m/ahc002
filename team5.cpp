...
#define DEBUG(...)                                                               \
    do                                                                           \
    {                                                                            \
        Debug::DEBUG(nintptr, "\033[3;35m#" + to_string(__LINE__) + "\033[m  "); \
        Debug::DEBUG(__VA_ARGS__);                                               \
        Debug::DEBUG(nintptr, "\033[m\n");                                       \
    } while (0);
...
public:
    ...
    void read()
    {
        ...
        for (Position pos = 0; pos < AREA; ++pos)
            std::cin >> tile_numbers[pos];
        for (Position pos = 0; pos < AREA; ++pos)
            std::cin >> this->_values[pos];
        ...
        for (Position row = 0; row < SIZE; ++row)
        {
            for (Position col = 0; col < SIZE; ++col)
            {
                ...
                if (this->tile_direction(p) >= 0)
                    continue;
                for (Direction dir = 0; dir < 2; ++dir)
                {
                    Point adj = Point(row + DRS[dir], col + DCS[dir]);
                    if (adj.row < 0 or adj.row >= SIZE)
                        continue;
                    if (adj.col < 0 or adj.col >= SIZE)
                        continue;
                    Position q = adj.id();
    ...
    {
        if (this->tile_direction(point) < 0)
            return point;
...
private:
    ...
    {
        if (dir == 0)
            return this->_data[point] = Type::Up;
        if (dir == 1)
            return this->_data[point] = Type::Right;
        if (dir == 2)
            return this->_data[point] = Type::Down;
        if (dir == 3)
            return this->_data[point] = Type::Left;
        assert(false);
    }
...
public:
    ...
    inline Point put(const Point &point, const Direction dir)
    {
        ...
        dev_assert(this->empty(p) and not this->terminated(point));
        dev_assert(this->empty(q));
    ...
    {
        auto p = point.id(), q = this->grid().same_point(point).id();
        dev_assert(this->directed(p));
        dev_assert(p == q or this->used(q));
    ...
    {
        if (point.row < 0 or point.row >= SIZE)
            return false;
        if (point.col < 0 or point.col >= SIZE)
            return false;
...
public:
    ...
    {
        if (this->out != nullptr)
            *this->out << this->build_answer(source) << "\n";
    }
...
public:
    ...
    inline bool apply(Board *const board, const Neighbor neighbor) const
    {
        ...
        do
        {
            start = random_engine(AREA);
        } while (not board->directed(start));
        ...
        Board *const best_board = nullptr) const
    {
        ...
        while (true)
        { // DFS
            if (++count % 100 == 0 and timer.expired())
                return (debug("timed out")), false;
            if (stack.empty())
                return (debug("stack empty")), false;

            auto motion = stack.top();
            stack.pop();
            if (motion >= 0)
            { // 上り
                ...
                {
                    if (this->grid.tile_direction(point) == shuffle[dir])
                        continue;
...
signed main()
{
    ...
    if constexpr (DEVELOPMEMT_MODE)
        std::clog << "Executed in development mode."
                  << "\n";
    else
        std::clog << "Executed in non-development mode."
                  << "\n";
