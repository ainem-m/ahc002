...
pub fn abs(a: usize, b: usize) -> ScoreType {
    if a > b {
        return (a - b) as ScoreType;
    }
    (b - a) as ScoreType
}
...
impl TileState {
    ...
    pub fn evaluateScore(&mut self) {
        ...
        let far_from_center = abs(24, self.pos_.i_).pow(2) + abs(24, self.pos_.j_).pow(2);
        self.evaluated_score_ += far_from_center;
    }
    ...
    /// 移動経路上の点p1からp2を繋ぎ直し、実際の点数が上がれば採用
    /// advance()を用いず直接seen_, steps_を変更する
    /// evaluateScore()も機能しなくなるので経路を作ってから行う
    /// (flagを立ててassertionする？)
    /// (0, 0) -R> (0, 1) -D> (1, 1) -R> (1, 2) -R> (1, 3)
    /// p1: 1 p2: 3のとき
    /// (0, 1)から(1, 2)に向かう新たなルートを探す
    /// start = (0, 1)
    /// before_steps = [(0, 1), (1, 1)]
    pub fn anneal(
        &mut self,
        rng: &mut ChaCha20Rng,
        input: &Input,
        ...
        let start = self.steps_[p1];
        let goal = self.steps_[p2];
        let before_score = self.game_score_;
        let before_steps = self.steps_.drain(p1..=p2).collect::<Vec<_>>();
        // start..=goalを未踏にし、点数もひく
        for &(i, j) in before_steps.iter() {
            self.seen_[input.tiles[i][j]] = false;
            self.game_score_ -= input.ps[i][j];
        }
        // たどり着いた経路の中で一番点数の良いものを採用
        let mut roots = vec![(before_score - self.game_score_, before_steps)];
        'dfs: for _ in 0..NUM_PATTERN {
            // 方向の優先度をシャッフルしてDFS
            let mut actions = (0..4).collect::<Vec<Action>>();
            actions.shuffle(rng);
            let i = start.0;
            let j = start.1;
            let mut steps = vec![];
            let mut stack = vec![(vec![], i, j)];
            let mut seen = self.seen_.clone();
            while let Some((mut step, i, j)) = stack.pop() {
                // 行きがけ
                step.push((i, j));
                if (i, j) == goal {
                    steps = step;
                ...
                if seen[input.tiles[i][j]] {
                    continue;
                }
                seen[input.tiles[i][j]] = true;
                for &action in actions.iter() {
                    let ni = i.wrapping_add(DIJ[action].0);
                    let nj = j.wrapping_add(DIJ[action].1);
                    if ni < TILE_SIZE && nj < TILE_SIZE && !seen[input.tiles[ni][nj]] {
                        stack.push((step.clone(), ni, nj));
                    }
                }
            }
            if steps.is_empty() {
            ...
            for i in 0..steps.len() - 1 {
                let di = steps[i + 1].0.wrapping_sub(steps[i].0);
                let dj = steps[i + 1].1.wrapping_sub(steps[i].1);
                if let Some(_) = DIJ.iter().position(|&(i, j)| i == di && j == dj) {
                    score += input.ps[steps[i].0][steps[i].1];
                } else {
                    panic!("skip!!{:?}", steps);
                }
            }

            roots.push((score, steps));
        }
        roots.sort();
        //eprintln!("271 {:?}", roots);
        let (score, steps) = roots.pop().unwrap();
        assert_eq!(steps[0], start);
        assert_eq!(steps[steps.len() - 1], goal);
        for &(i, j) in steps.iter() {
            self.seen_[input.tiles[i][j]] = true;
        }
        let steps_pre = &self.steps_[..p1];
        let steps_post = &self.steps_[p1..];
        self.steps_ = [steps_pre, &steps[..], steps_post].concat();
        self.game_score_ += score;
        self.turn_ = self.steps_.len() - 1;
    ...
    pub fn toString(&self, input: &Input) {
        ...
        for i in 1..self.turn_ {
            ...
            let di1 = self.steps_[i].0.wrapping_sub(self.steps_[i - 1].0);
            let dj1 = self.steps_[i].1.wrapping_sub(self.steps_[i - 1].1);
            let ind1 = DIJ.iter().position(|&(i, j)| di1 == i && dj1 == j).unwrap();
            dir.push(DIR[ind1]);
            let di2 = self.steps_[i + 1].0.wrapping_sub(self.steps_[i].0);
            let dj2 = self.steps_[i + 1].1.wrapping_sub(self.steps_[i].1);
            let ind2 = DIJ.iter().position(|&(i, j)| di2 == i && dj2 == j).unwrap();
            dir.push(DIR[ind2]);
...
fn chokudaiSearchActionWithTimeThreshold(
    ...
    if false && DEBUG {
        let mut max_beam = 0;
        eprintln!("{} beams:, ", beam.len());
        for t in 0..=beam_depth {
            eprint!("len = {}, ", beam[t].len());
            max_beam = max_beam.max(beam[t].len());
            for s in beam[t].iter().take(beam_width) {
                eprint!("{}, ", s.evaluated_score_);
            }
            eprintln!();
        }
        eprintln!("{}", max_beam);
...
pub fn step_to_output(steps: Vec<(usize, usize)>) {
    for i in 0..steps.len() - 1 {
        let di = steps[i + 1].0.wrapping_sub(steps[i].0);
        let dj = steps[i + 1].1.wrapping_sub(steps[i].1);
        if let Some(act) = DIJ.iter().position(|&(i, j)| di == i && dj == j) {
            print!("{}", DIR[act]);
        } else {
            panic!();
        }
...
fn main() {
    ...
    //while let Some(action) = chokudaiSearchAction(&input, &state, 3, 50, 50) {
    while let Some(action) = chokudaiSearchActionWithTimeThreshold(&input, &state, 3, 50, 0.0007) {
        loop_cnt += 1;
        if timekeeper.isTimeOver() {
            break;
        }
    ...
    while !timekeeper.isTimeOver() {
        loop_cnt += 1;
        let p1 = rng.gen_range(0, state.steps_.len() - 2);
        let p2 = (p1 + rng.gen_range(1, 5)).min(state.steps_.len() - 1);
        state.anneal(&mut rng, &input, 0.0, p1, p2);
    }
    if DEBUG {
        state.toString(&input);
    }
    step_to_output(state.steps_);
    eprintln!("{} loop, ", loop_cnt);
    eprintln!("{}", timekeeper.time());
    eprintln!("{}", state.game_score_);
}
